...
var handlers = {
    ...
    'LaunchRequest': function () {
        ...
        this.response.speak('Welcome to pet match. I can help you find the pet that is right for you. What kind of pet are you looking for?');
        this.response.listen('What kind of pet are you looking for?');
    ...
    'PetMatchIntent' : function () {
        ...
        httpGet(petMatchOptions).then(
            response => {
                ...
                this.response.speak(response.result.speech_output);
                this.response.cardRenderer('Your pet match is ...', `A ${response.result.animal.name}`,
                                            response.result.animal.images);
                //TODO we should not be composing what Alexa says in the API,
                // this should return the proper dog and then we compose the
                // prompt here.
            }
        ).catch( error => {
            console.log(error);
...
function buildPetMatchParams(slotValues) {
    let params = [
        ['animal', slotValues.animal.resolved],
        ['size', slotValues.size.resolved],
        ['shedding', slotValues.shedding.resolved],
